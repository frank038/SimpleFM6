...
def mmodule_type(mainLView):
    ...
    if shutil.which("archivemount"):
        if mainLView.selection:
            ...
            if imime.name() in ["application/vnd.rar","application/x-cd-image","application/zip","application/x-compressed-tar","application/x-tar","application/x-bzip-compressed-tar","application/x-xz-compressed-tar"]:
                return 1
    return 0
    ...
    def __init__(self, mainLView):
        ...
        if os.access(path, os.R_OK):
            try:
                ...
                os.mkdir(dest_dir)
                #
                ret = os.system("archivemount '{}' '{}' -o readonly,default_permissions". format(path, dest_dir))
                if ret == 0:
                    # add a button
